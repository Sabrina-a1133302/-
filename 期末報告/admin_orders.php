...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($status === '已出貨') {
        ...
        // 從資料庫撈取該訂單的 Email (假設資料表中有 email 欄位)
        $res = mysqli_query($link, "SELECT email FROM orders WHERE id = $id");
        $row_user = mysqli_fetch_assoc($res);
        
        if ($row_user) {
            // 呼叫 send_mail.php 中的函式
            // 'completed' 參數會觸發 send_mail.php 內對應的已出貨信件樣板
            send_order_notification($row_user['email'], '會員', $id, 'completed');
        }
    }
